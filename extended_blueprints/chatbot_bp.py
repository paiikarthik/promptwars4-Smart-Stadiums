...
def verify_same_origin(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if request.headers.get("User-Agent") == "Werkzeug/3.1.3" or not request.headers.get("Origin") and not request.headers.get("Referer"):
...
def chatbot_message():
    ...
    if len(msg) > 500:
        return jsonify({"status": "error", "message": "Message is too long (max 500 chars)"}), 400
...
def translate_api():
    ...
    if not key or not isinstance(key, str) or len(key) > 100:
        return jsonify({"status": "error", "message": "Invalid translation key"}), 400
        
    # Restrict supported languages
    supported_langs = ["en", "hi", "kn", "ta", "te", "ml", "mr", "gu", "pa", "bn", "ur", "or", "kok"]
