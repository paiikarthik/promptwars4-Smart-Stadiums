...
class ExtendedAppTestCase(unittest.TestCase):
    ...
    def test_chatbot_super_assistant(self):
        ...
        response = self.client.post("/extended/api/chatbot/message",
                                    data=json.dumps({"message": "What is today's match schedule?"}),
                                    content_type="application/json")
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertEqual(data["status"], "success")
        self.assertTrue("usa" in data["reply"].lower() or "england" in data["reply"].lower() or "fifa" in data["reply"].lower())

        # Posting message (sustainability query)
        response = self.client.post("/extended/api/chatbot/message",
                                    data=json.dumps({"message": "Tell me about green sustainability features"}),
                                    content_type="application/json")
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertEqual(data["status"], "success")
        self.assertTrue("sustainability" in data["reply"].lower() or "carbon" in data["reply"].lower() or "eco" in data["reply"].lower())
    ...
    def test_regional_translations(self):
        ...
        response = self.client.post("/extended/api/translation",
                                    data=json.dumps({"key": "lbl_match", "lang": "kn"}),
                                    content_type="application/json")
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertEqual(data["status"], "success")
        self.assertEqual(data["translated"], "ಇಂದಿನ ಪಂದ್ಯ") # Check exact match
