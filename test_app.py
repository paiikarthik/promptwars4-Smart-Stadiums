...
class ArenaFlowTest(unittest.TestCase):
    ...
    def test_flask_user_auth_flow(self):
        ...
        # 2. Login through API
        login_payload = {"username": "apiuser", "password": "apipassword"}
        response = self.client.post(
            "/api/auth/login",
    ...
    def test_admin_only_privileges(self):
        ...
        response = self.client.post(
            "/api/admin/broadcast",
            data=json.dumps(
                {"message": "Unauthorized broadcast", "type": "danger"}
            ),
            content_type="application/json",
        )
    ...
    def test_new_extensions_flow(self):
        ...
        db.register_user(
            "admin_user", generate_password_hash("adminpass"), "admin"
        )
        self.client.post(
            "/api/auth/login",
            data=json.dumps(
        ...
        # 4. Emergency SOS API
        sos_payload = {"seat": "Row F Seat 42", "zone_id": "zone-food"}
        response = self.client.post(
            "/api/sos/send",
