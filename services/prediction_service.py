...
class PredictionService:
    ...
    def get_predictions(self):
        ...
        for zone in zones:
            ...
            sec_deployed = len([d for d in dispatches.get("security", {}).get("deployed", []) if d.get("zone_id") == zone_id])
            med_deployed = len([d for d in dispatches.get("medical", {}).get("deployed", []) if d.get("zone_id") == zone_id])
            ...
            pred_5 = max(5, min(99, int(current_level + drift_5 - reduction_5)))
            
            # 10 Minute Prediction
            drift_10 = random.randint(-3, 4)
            reduction_10 = total_staff * 10 # 10% reduction in 10 min
            pred_10 = max(5, min(99, int(current_level + drift_10 - reduction_10)))
            
            # 30 Minute Prediction
            drift_30 = random.randint(-5, 5)
            reduction_30 = total_staff * 25 # 25% reduction in 30 min (fully resolved)
            pred_30 = max(5, min(99, int(current_level + drift_30 - reduction_30)))
